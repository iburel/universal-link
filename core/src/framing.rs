...
//! LSP-style framing: `Name: value\r\n`…, a blank line, then `Content-Length`
//! bytes of UTF-8 JSON. Line endings are `\r\n` (a lone `\n` is tolerated),
//! `Content-Length` is case-insensitive, unknown headers are ignored.
//!
//! Any violation is an error: the caller closes the connection. A peer that
//! does not speak the framing is owed no interpretable reply — fail-closed,
//! and nothing is allocated beyond the caps.
...
/// Reads a frame. `Ok(None)` = clean EOF between two frames; `Err` = a framing
/// violation or an I/O error (the caller closes).
pub async fn read_frame<R: AsyncBufRead + Unpin>(
    ...
    loop {
        line.clear();
        // Bounded read: at most what the header cap still allows (+1 to
        // distinguish "exactly at the cap" from "overflows").
        ...
        if !line.ends_with(b"\n") {
            return Err(violation("header line without a line ending"));
        }

        let text = str::from_utf8(&line).map_err(|_| violation("non-UTF-8 header"))?;
        ...
        if name.eq_ignore_ascii_case("content-length") {
            ...
            if len > MAX_FRAME_BYTES {
                return Err(violation("Content-Length beyond the cap"));
            }
    ...
    String::from_utf8(payload)
        .map(Some)
        .map_err(|_| violation("non-UTF-8 payload"))
}
