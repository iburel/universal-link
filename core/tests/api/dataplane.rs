...
/// relay of `device_id` (so that `files.send` can resolve and open it).
async fn wait_reachable(c: &mut TestComponent, device_id: &str) {
    wait_directory(
        c,
        device_id,
        |d| {
...
/// `device_id`.
async fn wait_attested(c: &mut TestComponent, device_id: &str) {
    wait_directory(
        c,
        device_id,
