...
| **Input engine** | spawned by the Core | The exclusive `input-backend` behind the routed `input.*` facade: lends this computer's keyboard and mouse to another one of the account, over the generic live channel (`peers.channel`). Per-OS backends read and consume the real devices and type on them (XInput2 with a grab plus XTEST on X11, the two low level hooks plus `SendInput` on Windows, a `CGEventTap` plus `CGEventPost` on macOS, the `InputCapture` and `RemoteDesktop` portals plus libei on Wayland); a session that cannot provide one keeps the component running and says which piece it is missing. The Wayland one is written and has never been run against a compositor, so it is switched off unless `ONEDEVICE_INPUT_WAYLAND` asks for it. Design and protocol in [input-sharing.md](input-sharing.md). |
